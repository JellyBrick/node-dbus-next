...
import { Message, sessionBus, Variant } from '@/index';
...
test('lists names on the bus', async () => {
  ...
  const names = (await call(iface, 'ListNames')) as string[];
...
test('get stats', async () => {
  ...
  const stats = (await call(iface, 'GetStats')) as Record<string, Variant>;
  expect(stats).toBeInstanceOf(Object);
  expect(stats).toHaveProperty('BusNames');
  const busNames = stats.BusNames;
  expect(busNames).toBeInstanceOf(Variant);
...
`;
  ...
  expect(hasMethod(iface, 'Frobate')).toBe(true);
  expect(hasMethod(iface, 'Bazify')).toBe(true);
  expect(hasMethod(iface, 'Mogrify')).toBe(true);
...
test('exported service introspection', async () => {
  ...
  const parser = createIntrospectParser();

  const introspect = async (path: string): Promise<{ node: { node: unknown[] } }> => {
    ...
    return parser.parse(typeof body === 'string' ? body : '') as { node: { node: unknown[] } };
  };

  const validateIntrospection = (
    introspection: { node: { node: unknown[] } },
    nodeCount: number,
  ): void => {
