...
import { Interface, property, method, signal, ACCESS_READ } from '@/service/interface';
...
class IntrospectionTestInterface extends Interface {
  ...
  myRenamedMethod(_what: number) {
    return 53;
  }

  @method({ inSignature: 'sd', outSignature: 'sd' })
  SomeMethod(str: string, d: number) {
    return [str, d];
  }
...
test('signal xml introspection', () => {
  ...
  signal = getSignal('RenamedSignal')[0];
  expect(signal).toBeDefined();
});
